...
export default function SalaryForm() {
  ...
  const { user, salaryData } = location.state || {};
  const navigate = useNavigate();

  const [formData, setFormData] = useState({
    trainerId: salaryData.trainerId || "",
    month: salaryData.month || "",
    basicSalary: salaryData.basicSalary || "",
    otHours: salaryData.otHours || "",
    otRate: salaryData.otRate || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/salaries/${salaryData._id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to edit salary");
      }
      ...
      console.error("Error editing salary:", error.message);
      toast.error(error.message || "Failed to edit salary!", {
        position: "top-center",
        autoClose: 2000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  return (
    <div>
      ...
      >
        <h4 className="text-warning text-center">Edit Salary</h4>
